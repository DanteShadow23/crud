...
def ingresar(nom, ape, id, fecha, gen):
    try:
         with conexion.cursor() as cursor:
            # Establecer la sentencia con datos variables
            consulta = "INSERT INTO usuario(nombre, apellido, identificacion, fecha_nacimiento, genero) VALUES (%s, %s, %s, %s, %s);"

            cursor.execute(consulta, (nom, ape, id, fecha, gen))
...
def actualizar(id, nom, ape, iden, fecha, gen):
    try:
        with conexion.cursor() as cursor:
            # En este caso no se necesita limpiar el dato
            consulta = "UPDATE usuario SET nombre = %s, apellido = %s, identificacion = %s, fecha_nacimiento = %s, genero = %s WHERE id = %s;"

            # Comando SQL valores a cambiar y llave primaria al final
            cursor.execute(consulta, (nom, ape, iden, fecha, gen, id))
...
def eliminar(id_num):
    try:
        with conexion.cursor() as cursor:
            ...
            # Comando SQL valores a cambiar y llave primaria al final
            cursor.execute(consulta, (id_num,))
